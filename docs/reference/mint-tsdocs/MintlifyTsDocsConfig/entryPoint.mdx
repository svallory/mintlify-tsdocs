---
title: "entryPoint"
icon: "file"
description: "Path to the main TypeScript declaration file (.d.ts) to document. This is typically the output of your TypeScript build process. If not specified, will auto-detect from package.json types or typings field."
---

import { Link } from '/snippets/tsdocs/Link.jsx';
import { PageLink } from '/snippets/tsdocs/PageLink.jsx';

# entryPoint

## Summary

Path to the main TypeScript declaration file (.d.ts) to document. This is typically the output of your TypeScript build process. If not specified, will auto-detect from package.json <code>types</code> or <code>typings</code> field.  

## Description

Path to the main TypeScript declaration file (.d.ts) to document. This is typically the output of your TypeScript build process. If not specified, will auto-detect from package.json or field.



## Examples

### Example 1

```typescript
&quot;./lib/index.d.ts&quot;
```
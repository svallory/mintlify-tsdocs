...
When you run `mint-tsdocs init`, it creates `.tsdocs/api-extractor.json` automatically. For manual setup:
...
# Or let mint-tsdocs do it
mint-tsdocs init  # Will offer to run 'mint new'
```
...
<Steps>
  ...
  <Step title="Run API Extractor">
    ```bash
    npx api-extractor run --local --config .tsdocs/api-extractor.json
    ls .tsdocs/*.api.json  # Should show .api.json file
    ```
  </Step>
...
**Solutions:**
- Verify .api.json exists in `.tsdocs/`

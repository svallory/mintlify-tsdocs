...
Running `mint-tsdocs` without any command defaults to `generate`:
...
5. Adds `"mint-ts"` script to package.json
...
# Skip api-extractor (use existing .api.json files)
...
3. Creates `.tsdocs/` cache directory
4. Generates `api-extractor.json` and `tsdoc.json` in `.tsdocs/`
5. Validates and compiles TypeScript
6. Runs api-extractor to generate `.api.json` files
7. Converts `.api.json` to MDX documentation
8. Updates Mintlify navigation in `docs.json`
...
**Options:**
- `-t`, `--template-dir PATH` - Template directory path (required)
...
Display configuration or cache statistics.

**Usage:**
```bash
mint-tsdocs show [TARGET] [OPTIONS]
mint-tsdocs show --target TARGET
```
...
mint-tsdocs show --target config

# Show cache statistics
mint-tsdocs show stats
mint-tsdocs show --target stats
```
...
- Type analysis cache (size, hits, misses, hit rate)
- API resolution cache (size, hits, misses, hit rate)
- Overall hit rate and total lookups
...
mint-tsdocs init ./packages/core
mint-tsdocs init ./packages/utils
mint-tsdocs init ./packages/plugins

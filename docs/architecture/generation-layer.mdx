...
The generation layer is the central orchestrator of `mint-tsdocs`. It traverses the API model and constructs the semantic structure of each documentation page using TSDoc AST nodes. The layer has been refactored to use modular components for navigation management, caching, and performance monitoring.
